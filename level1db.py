...
from mpt import keccak_hash, NibblePath, Node
...
class Level1db:
    ...
    def delete(self, encoded_key):
        ...
        if action == Level1db._DeleteAction.DELETED:
            # Trie is empty
            self._root = None
        elif action == Level1db._DeleteAction.UPDATED:
            new_root = info
            self._root = new_root
        elif action == Level1db._DeleteAction.USELESS_BRANCH:
            _, new_root = info
            self._root = new_root
    ...
    def _delete(self, node_ref, path):
        ...
        if type(node) == Node.Leaf:
            ...
            if path == node.path:
                return Level1db._DeleteAction.DELETED, None
            else:
                raise KeyError
            ...
            if action == Level1db._DeleteAction.DELETED:
                ...
                return Level1db._DeleteAction.UPDATED, new_reference
            ...
            if len(path) == 0 and len(node.data) == 0:
                ...
                node.data = b''
                action = Level1db._DeleteAction.DELETED
            else:
            ...
            if action == Level1db._DeleteAction.DELETED:
                ...
                if non_empty_count == 0 and len(node.data) == 0:
                    ...
                    return Level1db._DeleteAction.USELESS_BRANCH, (path, reference)
                    ...
                    return Level1db._DeleteAction.UPDATED, reference
            elif action == Level1db._DeleteAction.UPDATED:
